...
# 2022.04.29 : find and add the skip file list
good_locs = ['output/linked_good_0',]
previous_list = []
for loc in good_locs:
    for name in glob(f'{loc}/*.root'):
        previous_list.append(name.split('/')[-1][:-5])
...
all_ofiles = glob('tmplogs/*.log')
sums_ofiles = []
for name in all_ofiles:
    which = name.split('/')[-1][:-4]
    # print("which: ", which)
    if which not in previous_list:
        sums_ofiles.append(name)
    else:
        print(f"already had {name}")
# exit(2)
...
with open ('sums_statuses_update.txt','w') as f_out:
